...
class LiveActivityManager: ObservableObject {
  ...
  @Published var currentActivity: Activity<DiaumWidgetAttributes>?
  ...
  private func restoreExistingActivity() {
    ...
    if !storedActivityId.isEmpty {
      if let existingActivity = Activity<DiaumWidgetAttributes>.activities.first(where: {
  ...
  func startSessionActivity(session: BlockedProfileSession) {
    ...
    let attributes = DiaumWidgetAttributes(name: profileName, message: message)
    let contentState = DiaumWidgetAttributes.ContentState(
  ...
  func updateSessionActivity(session: BlockedProfileSession) {
    ...
    let updatedState = DiaumWidgetAttributes.ContentState(
      startTime: session.startTime,
      isBreakActive: session.isBreakActive
    )
  ...
  func endSessionActivity() {
    ...
    // End the activity
    let completedState = DiaumWidgetAttributes.ContentState(
      startTime: Date.now
    )
